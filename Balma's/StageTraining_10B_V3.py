...
class StageTraining_10B_V3(Task):
    ...
    def __init__(self):
        ...
        --> Opto included
        --> Choice variable (Opto type) defines if session has opto or not (0 OFF, 6 ON)

        ########   PORTS INFO   ########
        Port 1 - WATER PORT: LED, photogates and pump
        Port 2 - BUZZER: valve (16kHz): correct; LED (4kHz):punish
        Port 3 - PHOTOGATES 4: Photogates end of corridor   #### if there are problems try changing to PAm4
    ...
    def init_variables(self):
        # general
        self.duration_max = 4500  # 1h:15 min finished the task
        ...
        self.opto_type = 0  # 0 off, 6 for inhibition, 7 for activation
        self.choice = 0     # is opto type but comes form subjects.csv
        self.opto_bool = 0  # trial with light on/off
        self.opto_fail = np.nan  # opto trial aborted
        ...
        pulse_dur_off = 1.0 / float(self.frequency_light) - self.pulse_dur_on

        # pulse pal
        self.pulse_pal = PulsePal(address='/dev/pulsepal')


        # by default samples_per_second = 1000
        # if the pulse is long we can't store so many samples, we need samples_per_second = 500
        pulse1 = self.pulse_pal.create_square_pulse(self.max_dur_light, 0, 0, 5,  #duration, duration_ramp_in, duration_ramp_off, voltage
                                                    samples_per_second=500)
        pulse2 = self.pulse_pal.create_square_pulsetrain(self.max_dur_light,
                                                         self.pulse_dur_on,
                                                         pulse_dur_off,
                                                         3,  #duration, time_on, time_off, voltage
                                                         samples_per_second=500)
    ...
    def configure_gui(self):  # Variables appearing in the GUI
        self.gui_input = ['stage', 'substage', 'duration_max', 'stim_dur_ds', 'stim_dur_dm', 'stim_dur_dl', 'choice', 'silent']
    ...
    def main_loop(self):
        ...
        #  OPTO STIMULATION
        if self.current_trial > 10 and self.opto_type != 0 and self.last_trial_result in ['correct_first',
                                                                                        'correct_other']:
            self.opto_bool = np.random.choice([0, 1], p=[0.8, 0.2])  # 20% trials laser ON
            if self.opto_bool == 1:
        ...
        # Other trials
            ...
            if self.opto_bool == 1:
                ...
                self.sma.add_state(
                    state_name='Start_task',
                    state_timer=10,
                    state_change_conditions={Bpod.Events.Port1Out: 'Light_on',
                                             Bpod.Events.Tup: 'Start_task2'},
                    output_actions=[])
                ...
                self.sma.add_state(
                    ...
                    output_actions=[(Bpod.OutputChannels.SoftCode, self.opto_type)])  # turn on opto light (6=inactivation, 7=activation)
    ...
    def after_trial(self):
        ...
        # correct opto bool in invalids
        try:
            if self.current_trial_states['Light_on'][0][0] > 0:
                self.opto_fail = 0
            elif self.opto_bool == 1:
                self.opto_fail = 1
            else:
                self.opto_fail = np.nan
        except:
            self.opto_fail = np.nan

        # End-trial calculations
        self.last_x = self.x
        self.last_trial_result = self.trial_result
        ...
        self.register_value('opto_fail', self.opto_fail)
