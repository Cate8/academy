...
class S4_5(Task):
    ...
    def init_variables(self):
        ...
        self.duration_max = 60*60  # 45 min finished the task 
        self.duration_min = 45*60  # 45 min door opens 
        ...
        def generate_probs_vec(N_blocks, prob_block_type, p_list, prob_Left_Right_blocks):
            ...
            if((N_blocks % 2) == 1):
                N_blocks = N_blocks +1
                Warning('We increased the number of blocks by one to have an even number')
            ...
            if (prob_block_type == 'rdm_values'):
                ...
                if (prob_Left_Right_blocks == 'indep'):  # in this condition, the Right and Left blocks have independent probabilities
                    prob_list_Left_blocks = 1. - np.squeeze(np.random.choice(p_list, (N_blocks_by_half, 1)))
                elif (prob_Left_Right_blocks == 'balanced'):  # in this condition, the Left blocks have 1 - p_Right but in a random order, so that the unconditional p_Right across the session is 0.5
                ...
                times_rep_per = np.floor(N_blocks_by_half / N_probs).astype(int) # we fill the prob_list_blocks135 array with as many repetitions from per as we can fit.
                print('times_rep_per = ', times_rep_per)
                for i in range(times_rep_per):
                    per_Right_probs = np.random.permutation(p_list) #for each repetition we generate a new permutation for ODD blocks 1, 3, 5, ....
                    per_Left_probs = 1. - np.random.permutation(p_list) #for each repetition we generate a new permutation for ODD blocks 2, 4, 5, ....
                    prob_list_Right_blocks[0 + i * N_probs:0 + (i + 1) * N_probs] = per_Right_probs
                    prob_list_Left_blocks[0 + i * N_probs:0 + (i + 1) * N_probs] = per_Left_probs
                ...
                per_Right_probs = np.random.permutation(p_list) #we generate a final permutation
                per_Left_probs = 1. - np.random.permutation(p_list) 
                for i in range(np.remainder(N_blocks_by_half, N_probs)):
                    prob_list_Right_blocks[N_probs * times_rep_per + i] = per_Right_probs[i]
                    prob_list_Left_blocks[N_probs * times_rep_per + i] = per_Left_probs[i]
            else:
                Warning('Specify the way to take prob values from prob_list: rdm_values or permutation_prob_list ')

            #Assign Right and Left probs to the output vector depending on the order rdm chosen by rdm_right_block_order
            rdm_right_block_order = np.random.permutation([0, 1])  # the first position of this array rdm_right_block_order[0] ...
