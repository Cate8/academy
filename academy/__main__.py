...
def main_loop():
    ...
    while True:
        ...
        if status == 't':
            ...
            try:
                temperature = float(tag[0:5])
                humidity = float(tag[6:11])

                utils.log('Academy', 'Temperature: ' + str(temperature) + 'C, Humidity: ' + str(humidity) + '%',
                          'ACTION')

                utils.temperatures.add_new_item({'temperature': temperature,
                                                 'humidity': humidity})

                if temperature > float(settings.MAXIMUM_TEMPERATURE):
                    telegram_bot.alarm_temperature(str(temperature))
                if temperature < float(settings.MINIMUM_TEMPERATURE):
                    telegram_bot.alarm_temperature(str(temperature))
            except:
                pass
        ...
        if utils.state == 0:  # waiting
            ...
            if status == 's':
                try:
                    ...
                    subject_action(first_time=True, last_tags=last_tags)
                except:
                    utils.log_cam('Academy', 'Subject not found for tag: ' + tag, 'ERROR')

            elif status is None:
                if time_utils.now_seconds() < last_time_tag + settings.DURATION_TAG:
                    subject_action(first_time=False, last_tags=last_tags)

        elif utils.state == 1:  # 1 before first action
            if status == 's':
                utils.log_cam(utils.subject.name, 'Not allowed to exit, task has not started yet', 'ACTION')
            ...
            elif time_utils.now_seconds() > utils.alarm_trapped_time + 10 and doors1 > settings.NOMICEDOOR1:
            ...
            if status == 'w':
                try:
                    ...
                    if len(weight) == 5 and cam1.area_doors2.value < settings.NOMICEDOOR2 and cam1.area_doors1.value > settings.NOMICEDOOR2:
                        ...
                        weight = []
                        utils.change_to_state = 4  # after min_time, data not saved, animal back
            ...
            if utils.chrono.get_seconds() >= utils.task_real_duration + utils.alarm_mouse_time:
                bpod.play_buzzer1()
                bpod.play_buzzer2()
                arduino.noise_door2()
                bpod.play_buzzer1()
            ...
            if utils.chrono.get_seconds() >= utils.task_real_duration + utils.alarm_mouse_time2:
                try:
                    telegram_bot.alarm_session_time(utils.task_real_duration + utils.alarm_mouse_time2, utils.subject.name)
                except:
                    telegram_bot.alarm_session_time(utils.task_real_duration + utils.alarm_mouse_time2, '')
                utils.alarm_mouse_time2 += 3600

            if time_utils.now_seconds() > last_time_scale + 600 \
                    and cam1.area_doors1.value < settings.NOMICEDOOR1 and cam1.area_total.value < settings.NOMICEDOOR1:
        ...
        elif utils.state == 10:  # multiple animals inside, data not saved, animal not back
            stop_and_save_task()
...
def stop_and_save_task():
    ...
    if utils.task_manager is not None:
        try:
            ...
            print('no data to save session stopped: ', error)
    ...
    name_task = utils.task.task + '-' + str(int(float(utils.task.stage))) + '-' + str(int(float(utils.task.substage)))
...
def screen_loop():
    ...
    if trials_performed >= utils.task.trials_max or utils.chrono.get_seconds() >= utils.task.duration_max:
        if utils.state == 8:
    ...
    if utils.state == 2 or utils.state == 1:
        ...
        if (seconds > utils.task.duration_tired and len(last_trials) < utils.task.trials_tired) or utils.task.tired:
            utils.change_to_state = 3  # 3 after min_time, data not saved, animal not back
        if (seconds > utils.task.duration_min and trials_performed >= utils.task.trials_min):
            utils.change_to_state = 3  # 3 after min_time, data not saved, animal not back

    if cam2.tracking_inside:
        if time_utils.now_seconds() > utils.alarm_mice_time + 10 and cam2.area_total.value > settings.SEVERALMICE and utils.state != 8:
    ...
    if cam3.tracking_inside:
        if time_utils.now_seconds() > utils.alarm_mice_time + 10 and cam3.area_total.value > settings.SEVERALMICE and utils.state != 8:
            utils.alarm_mice_repetition += 1
            utils.alarm_mice_time = time.time()
            if utils.alarm_mice_repetition > 3:
                utils.alarm_mice_repetition = 0
        ...
        if settings.CAM3_FLOOR_ON:
            if time_utils.now_seconds() > utils.alarm_mice_time + 10 and cam3.area_total_floor.value > settings.FLOORMOUSE and utils.state != 8:
                utils.alarm_mice_repetition += 1
                utils.alarm_mice_time = time.time()
                if utils.alarm_mice_repetition > 3:
                    utils.alarm_mice_repetition = 0
...
def subject_action(first_time, last_tags):
    ...
    if cage > settings.NOMICECAGE:
        if first_time:
            utils.log_cam(utils.subject.name, 'Movement in the cage area ' + str(cage), 'ACTION')
        ...
        if time_utils.now_datetime() > utils.super_subject.min_time or utils.super_subject.all_in:
            ...
            if first_time:
                utils.log_cam(utils.subject.name,
                              'Not allowed to enter until: ' + time_utils.datetime_to_string(utils.super_subject.min_time),
                              'ACTION')
...
def go_to_state(num):
    ...
    if num == 0:  # waiting
        ...
        try:
            ...
            utils.relaunch = True
            telegram_bot.alarm_starting_task(name_task, error, utils.subject_name)
            print(traceback.format_exc())
            arduino.open_door2()
            utils.change_to_state = 6
        ...
        utils.subject_trapped = False
        arduino.close_door2()
        utils.log_cam(utils.subject.name, 'First action in box, closing door 2', 'ACTION')
