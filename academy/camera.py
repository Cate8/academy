...
class Video(Process):
    def __init__(self, port='0', cam_number=0, name_video=None, path=None, width=None, height=None, fps=None,
                 ...
                 threshold=0, threshold_mouse = 0, threshold_led=0,
                 cage_zone=None, doors1_zone=None, doors2_zone=None, doors3_zone=None, floor1_zone=None, floor2_zone=None,
        ...
        self.threshold_mouse = threshold_mouse
        self.threshold_led = threshold_led
        self.threshold2 = 0
        self.threshold3 = 0
        ...
        self.doors3_zone = doors3_zone if doors3_zone is not None else [640, 0, 480, 0]
    ...
    def do_tracking_in(self):
        ...
        if self.tracking_inside:
            [zdxmin1, zdxmax1, zdymin1, zdymax1] = self.doors1_zone
            [zdxmin2, zdxmax2, zdymin2, zdymax2] = self.doors2_zone
            [zdxmin3, zdxmax3, zdymin3, zdymax3] = self.doors3_zone
            ...
            try:
                doors3_greyscale_frame = cv2.cvtColor(self.frame[zdymin3:zdymax3, zdxmin3:zdxmax3], cv2.COLOR_BGR2GRAY)
                doors3_gaussian_frame = cv2.GaussianBlur(doors3_greyscale_frame, (5, 5), 0)
                doors3_thresh = cv2.threshold(doors3_gaussian_frame, self.threshold, 225, cv2.THRESH_BINARY_INV)[1]
                area_doors3 = cv2.countNonZero(doors3_thresh)
            except:
                area_doors3 = 0


            area_total = area_doors1 + area_doors2 + area_doors3
            self.area_doors1.value = int(area_doors1)
            self.area_doors2.value = int(area_doors2)
            self.area_total.value = int(area_total)
    ...
    def create_mask(self):
        if self.create_mask_flag:
            ...
            cv2.rectangle(self.mask_mouse, (zdxmin3, zdymin3), (zdxmax3, zdymax3), (255, 255, 255), -1)
    ...
    def do_tracking_position(self):
        if self.tracking_position:
            ...
            #mask_mouse = np.zeros_like(gray_frame)
            #cv2.rectangle(mask_mouse, (zdxmin1, zdymin1), (zdxmax1, zdymax1), (255, 255, 255), -1)
            #cv2.rectangle(mask_mouse, (zdxmin2, zdymin2), (zdxmax2, zdymax2), (255, 255, 255), -1)
            #mask_mouse2 = cv2.bitwise_not(mask_mouse)
            ...
            _, thresh = cv2.threshold(frame_mouse, self.threshold_mouse, 255, cv2.THRESH_BINARY_INV) # Adjust threshold value as needed
            ...
            for index, contour in enumerate(contours):
                ...
                self.max_luminance_led = np.max(masked_image)

                if self.max_luminance_led > self.threshold_led:
                    self.led_on = True
                else:
                    self.led_on = False
    ...
    def add_info_tracking_in(self):
        if self.tracking_inside:
            try:
                ...
                cv2.rectangle(self.frame, (zdxmin3, zdymin3), (zdxmax3, zdymax3), (255, 0, 255), 2)
                for key, value in self.centers.items():
                    cv2.circle(self.frame, value, 7, (255, 255, 255), -1)
    ...
    def add_info_tracking_position(self):
        if self.tracking_position:
            ...
            if self.led_on:
                cv2.circle(self.frame, (75, 130), 20, (255, 255, 0), -1)

            cv2.putText(self.frame, 'threshold mouse: ' + str(self.threshold_mouse), (10, 200),
                        cv2.FONT_HERSHEY_DUPLEX, 0.4, (0, 255, 0), 1, cv2.LINE_AA)

            cv2.putText(self.frame, 'max luminance led: ' + str(self.max_luminance_led), (10, 220),
                        cv2.FONT_HERSHEY_DUPLEX, 0.4, (0, 255, 0), 1, cv2.LINE_AA)
    ...
    def save_video3(self):
        ...
        arrays = [self.timestamps, self.frames, self.trials, self.states, self.cxs, self.cys, self.led_ons]
        ...
        self.led_ons = self.led_ons[:min_length]
        ...
        df = pd.DataFrame({'timestamps': timestamps, 'frames': frames, 'trial': self.trials, 'states': states,
                           'x': self.cxs, 'y': self.cys, 'led_on': self.led_ons})
...
try:
    ...
    cam3 = Video(port=settings.CAMERA3_PORT,
                 ...
                 threshold_mouse=settings.CAM3_THRESHOLD_MOUSE,
                 threshold_led=settings.CAM3_THRESHOLD_LED,
                 cage_zone=settings.CAM3_CAGE_ZONE,
                 doors1_zone=settings.CAM3_DOORS1_ZONE,
                 doors2_zone=settings.CAM3_DOORS2_ZONE,
                 doors3_zone=settings.CAM3_DOORS3_ZONE,
